...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<tr>
				...
				<th>
					<span class="requiredField">*</span>行业父节点:
				</th>
				<td>
					<select name="parentId" [#if hasQuestion]disabled[/#if]>
						<option value="">请选择父节点...</option>
						[#list industryCategoryTree as category]
							[#if category != industryCategory ]
								<option value="${category.id}"[#if category == industryCategory.parent] selected="selected"[/#if]>
									[#if category.grade != 0]
			...
			<tr>
				...
				<td>
					<input type="text" name="order" id="order" class="text" value="${industryCategory.order}" maxlength="9" />
				</td>
			</tr>
		</table>
