...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a>[#if materialID??] &raquo; 添加材料子题[/#if] &raquo; 添加填空题
	</div>
	<form id="inputForm" action="save.jhtml" method="post" enctype="multipart/form-data">
		<input type="hidden" name="submitType" id="submitType" />
		<input type="hidden" name="questionType" id="questionType" value="blanks"/>
		<input type="hidden" name="questionType" value="blanks" />
		<input type="hidden" name="materialID" id="materialID" value="${materialID}"/>
		<input type="hidden" name="treePath2" id="treePath" value="${materialID}"/>
