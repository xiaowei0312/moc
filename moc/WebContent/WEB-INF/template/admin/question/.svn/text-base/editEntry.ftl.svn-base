...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="outlineCategoryId" value="${question.outlineCategory.id}"/>
		...
		<table class="input tabContent">
			...
			[#list arr as a]
				<tr>
					<th>
						<span class="requiredField">*</span>${a[0]}:
					</th>
					<td>
					<input type="hidden" name="keys" value="${a[0]}"/>
							<select name="borrowselect" >
							[#list accoutCatalogs as category]
									<option value="${category.code}"   [#if category.code == a[1] ] selected [/#if]>
										[#if category.grade != 0]
											[#list 1..category.grade as i]
												&nbsp;&nbsp;
											[/#list]
						...
						<input type="text"  name="borrowanswer" class="text"  maxlength="200" value="${a[2]}"/>
						<input type="button" value=" - "  onClick="$(this).closest('tr').remove();">
					</td>
				</tr>
			[/#list]
