...
(function($) {
	...
	$.dialogIframe = function(options) {
		...
		var dialogContentHeight;
		if (settings.iframeSrc == null) {
			return false;
		}else{
			dialogContentHeight = settings.height - 80;
		}
		...
		if (settings.type != null) {
			...
			$dialogContent = $('<div class="dialogContent" style="height:' + dialogContentHeight +'px"><\/div>').appendTo($dialog);
		}
		
		if (settings.type != null) {
			$dialogIframe = $('<iframe id="dialogIframe" name="dialogIframe" scrolling="yes" class="dialogContent dialog' + settings.type + 'Icon" src="'+ settings.iframeSrc +'"><\/iframe>').appendTo($dialogContent);
		} else {
			$dialogIframe = $('<iframe id="dialogIframe" name="dialogIframe" scrolling="yes" style="overflow-x:hidden;width:100%;height:' + dialogContentHeight+'px;" src="'+ settings.iframeSrc +'"><\/iframe>').appendTo($dialogContent);
		}
		...
		if ($dialogOk != null) {
			$dialogOk.click(function() {
				if (settings.onOk && typeof settings.onOk == "function") {
					if (settings.onOk(window.frames["dialogIframe"]) != false) {
						dialogClose();
					}
				} else {
					dialogClose();
		...
		if ($dialogCancel != null) {
			$dialogCancel.click(function() {
				if (settings.onCancel && typeof settings.onCancel == "function") {
					if (settings.onCancel($dialogIframe) != false) {
						dialogClose();
					}
				} else {
					dialogClose();
		...
		function dialogShow() {
			if (settings.onShow && typeof settings.onShow == "function") {
				if (settings.onShow($dialogIframe) != false) {
					$dialog.show();
					$dialogOverlay.show();
				}
			} else {
		...
		function dialogClose() {
			if (settings.onClose && typeof settings.onClose == "function") {
				if (settings.onClose($dialogIframe) != false) {
					$dialogOverlay.remove();
					$dialog.remove();
				}
			} else {
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: sram.base + "/admin/file/browser.jhtml",
				uploadUrl: sram.base + "/admin/file/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = sram.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = sram.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
