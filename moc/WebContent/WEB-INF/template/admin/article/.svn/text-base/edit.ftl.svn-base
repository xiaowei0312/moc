...
<title>${message("admin.article.edit")} - Powered By Sram</title>
					...
					<select name="articleCategoryId" >
						[#list articleCategoryTree as articleCategory]
							<option value="${articleCategory.id}"
								[#if articleCategory == article.articleCategory] 
									selected="selected"
								[#elseif articleCategory.children?? && articleCategory.children?size>0]
									disabled=""
								[/#if]>
