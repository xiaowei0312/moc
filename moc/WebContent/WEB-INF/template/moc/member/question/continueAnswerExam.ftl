...
<head>
    ...
    <script type="text/javascript">
        var limitedTime = parseInt(${limitedTime});
        //分录题的值格式化
        ...
        $(function() {
            ...
            $(".timer").countDown({
                ...
                callback_doEnd: function() { //计时结束回调
                    layer.alert("该交卷了！", 11, ['提示', 'background:#60B979;']);
                },
                callback_timeOut: function(times) { //超时后每秒回调
                    $(this).html("-" + times.minute + ":" + times.second);
                    $("input[name=usedTime]").val(limitedTime+times.total);
                    if (times.total % 300 == 0) {
                        layer.alert("您已超时" + times.total / 60 + "分钟，请掌握好时间，尽快交卷", 11, ['提示', 'background:#60B979;']);
                    }
                }
            });
        //分录题的增加按钮事件  
			...
			'<div>
			    ...
			    <select name="entry_select_code" onChange="entryFormat(this);">
			        <option value="">---请选择---<\/option>
						...
						[/#list]
			    <\/select>
			    <input name="entry_input_value" onKeyup="entryFormat(this);">
		...
		$(this).closest("div").after(elements);
            ...
            $('[id^="myTab3_Content"]').each(function(i) {
                ...
                $(this).children('[id^="item_"]').each(function(i) {
                   $(this).children().find('[name^="testpaperItem_"]').each(function(m) {
                		var itemID = $(this).attr('name').split("_")[3];
                   		var type=$(this).attr('type');
                   		if($(this).attr("checked")=="checked" || (type=="text" && $(this).val()!="") || ($(this).is("textarea") && $(this).val()!="") ){
                 			 $("#fastto_item_"+itemID).addClass("over_answer");
                 			 zuoqiCount++;
                 			 return false;
                   		}
                ...
                $('.count_' + i).prev().html(itemCount - clCount-zuoqiCount);
            });
        ...
        <div class="examtext">
            ...
            <div class="left_nemu" style="min-height:600px">
                ...
                <form name="oldexamForm" id="oldexamForm" action="saveOldexm.jhtml" method="post">
                    ...
                    <input type="hidden" name="lastusedTime" value="${lastusedTime}"/>
                        ...
                        <div class="itemlist" id="item_${testpaperItem.id}">
                            <p>
	                            ...
	                            <span class="quename">
		                            ...
		                            [/#if]
		                            [#if testpaperItem.question.questionType == 'blanks'  ]
		                            	[#if testpaperItem.testpaperItemResult??]${testpaperItem.testpaperItemResult.stemToBlanks}[/#if]
                                       	<a class="dblink1 [#if questionFavoriteIds?seq_contains(testpaperItem.question.id)] qx [#else] sc[/#if]"  id="${testpaperItem.question.id}">[#if questionFavoriteIds?seq_contains(testpaperItem.question.id)] 取消收藏 [#else] 收藏本题 [/#if]</a>
		                            [/#if]
	                            </span>
                            </p>
                            ...
                            <div class="options">
                            	<ul>
                            	    [#assign choiceAnswer=testpaperItem.testpaperItemResult.answer /]
                            		[#list testpaperItem.question.toMetas as meta]
		                                <li class="option_content" id="testpaperItem_${testpaperChapter.id}_${testpaperItem.question.id}_${testpaperItem.id}_${meta?substring(0,1)}">
		                                	[#assign choiceOption=meta?substring(0,1) /]
									    	<span class="option_index [#if	choiceAnswer?? && choiceAnswer?index_of(choiceOption)!=-1] optionSelected [/#if]">${meta?substring(0,2)}</span>
                            ...
                            [#if testpaperItem.question.questionType = 'single_choice' || testpaperItem.question.questionType = 'choice' || testpaperItem.question.questionType = 'uncertain_singlechoice' || testpaperItem.question.questionType = 'uncertain_choice' ]
	                            <span class="ansright spanright no">
		                            ...
		                            [#list testpaperItem.question.toMetasChar as mchar]
		                                <span class="ansLable"><input type="[#if testpaperItem.question.questionType = 'single_choice']radio[#else]checkbox[/#if]" 
		                                	 name="testpaperItem_${testpaperChapter.id}_${testpaperItem.question.id}_${testpaperItem.id}"
		                                	 [#if	choiceAnswer?? && choiceAnswer?index_of(mchar)!=-1] checked="checked" [/#if]
		                                	 value="${mchar}"/>
                            ...
                            [#if testpaperItem.question.questionType = 'determine' ]
	                            <span class="ansright spanright no ew">
	                            	 [#assign determineAnswer=testpaperItem.testpaperItemResult.answer /]
			                         <span class="ansLable"><input type="radio"  name="testpaperItem_${testpaperChapter.id}_${testpaperItem.question.id}_${testpaperItem.id}" [#if	determineAnswer?? && determineAnswer?index_of("true")!=-1] checked="checked" [/#if] value="true"><label for="">正确</label></span>
			                         <span class="ansLable"><input type="radio"  name="testpaperItem_${testpaperChapter.id}_${testpaperItem.question.id}_${testpaperItem.id}" [#if	determineAnswer?? && determineAnswer?index_of("false")!=-1] checked="checked" [/#if] value="false"><label for="">错误</label></span>
			                        <a class="dblink1 [#if questionFavoriteIds?seq_contains(testpaperItem.question.id)] qx [#else] sc[/#if]"  id="${testpaperItem.question.id}">[#if questionFavoriteIds?seq_contains(testpaperItem.question.id)] 取消收藏 [#else] 收藏本题 [/#if]</a>     
			                    </span> 
		                    [/#if] 
                    		[#if testpaperItem.question.questionType = 'essay' ]
                            	<span class="ansright spanright no et">
                            	<textarea class="textfocus" name="testpaperItem_${testpaperChapter.id}_${testpaperItem.question.id}_${testpaperItem.id}">${testpaperItem.testpaperItemResult.essayAnswer}</textarea>
                       		 	</span>
                            [/#if]
                            <!-- 材料题-->
                            [#if testpaperItem.question.questionType = 'material' ]
	                            ...
	                            <div class="answer" style="padding-left:0;">
	                                ...
	                                [#if children.question.questionType != 'blanks'  ]${children.question.stem}[#else][#if testpaperItem.testpaperItemResult??]${children.testpaperItemResult.stemToBlanks}[/#if][/#if]</p>
	                                ...
	                                <div class="options">
	                                    <ul>
	                                	    [#assign choiceAnswer=children.testpaperItemResult.answer /]
	                                ...
	                                <span class="ansright spanright no">
                                        ...
                                        [#list children.question.toMetasChar as mchar]
                                           <span class="ansLable"><input type="[#if children.question.questionType = 'single_choice']radio[#else]checkbox[/#if]"  name="testpaperItem_${testpaperChapter.id}_${children.question.id}_${children.id}" 
                                           		[#if choiceAnswer?? && choiceAnswer?index_of(mchar)!=-1] checked="checked" [/#if]
                                           		value="${mchar}"/>
                                           <label for="">${mchar}</label></span>
                                        [/#list]
	                                ...
	                                <span class="ansright spanright no">
		                            	 [#assign determineAnswer=children.testpaperItemResult.answer /]
                                        <span class="ansLable"><input type="radio" name="testpaperItem_${testpaperChapter.id}_${children.question.id}_${children.id}" [#if determineAnswer?? && determineAnswer?index_of("true")!=-1] checked="checked" [/#if] value="true"><label for="">正确</label></span>
                                        <span class="ansLable"><input type="radio" name="testpaperItem_${testpaperChapter.id}_${children.question.id}_${children.id}" [#if	determineAnswer?? && determineAnswer?index_of("false")!=-1] checked="checked" [/#if] value="false"><label for="">错误</label></span>
                                        </span> 
	                                [/#if]
	                                <!--问答-->
	                                [#if children.question.questionType = 'essay'  ]
	                               	    <span class="ansright spanright no et">
	                                        <textarea class="textfocus" name="testpaperItem_${testpaperChapter.id}_${children.question.id}_${children.id}">${children.testpaperItemResult.essayAnswer}</textarea>
	                                    </span> 
	                                [/#if] 
		                                <!--分录题-->
		                                [#if children.question.questionType = 'entry' ]
			                                <span class="ansright spanright no">
			                                		[#if children.testpaperItemResult.entryAnswerArrList??]
				                                		[#list children.testpaperItemResult.entryAnswerArrList as entryAnswerArr]
			                                				<div>
				                                				<select name="entry_select_mark" onChange="entryFormat(this);">
															        <option value="借" [#if entryAnswerArr[0]='借'] selected="selected"  [/#if]>借</option>
															        <option value="贷" [#if entryAnswerArr[0]='贷'] selected="selected"  [/#if]>贷</option>
															    </select>
															    <select name="entry_select_code" onChange="entryFormat(this);">
															        <option value="">---请选择---</option>
																	[#list accoutCatalogs as category]
																		<option value="${category.code}" [#if entryAnswerArr?size gte 2 && category.code=entryAnswerArr[1]] selected="selected" [/#if]>
																			[#if category.grade != 0]
																				[#list 1..category.grade as i]
																					&nbsp;&nbsp;
																				[/#list]
															    ...
															    <input name="entry_input_value" onKeyup="entryFormat(this);" AUTOCOMPLETE="off" [#if entryAnswerArr?size gte 3] value="${entryAnswerArr[2]}" [/#if]/>
															    <input type="hidden" name="testpaperItem_${testpaperChapter.id}_${children.question.id}_${children.id}" value="${entryAnswerArr[0]},[#if entryAnswerArr?size gte 2]${entryAnswerArr[1]}[/#if],[#if entryAnswerArr?size gte 3]${entryAnswerArr[2]}[/#if]"/>
															    <input type="button" value=" + " class="plus">
																<input type="button" value=" - " onClick="$(this).closest('div').remove();">
															</div>
				                                		[/#list]
