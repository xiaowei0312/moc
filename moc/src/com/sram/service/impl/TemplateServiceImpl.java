...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			File shopxxXmlFile = new ClassPathResource(CommonAttributes.SRAM_XML_PATH).getFile();
			Document document = new SAXReader().read(shopxxXmlFile);
			List<Template> templates = new ArrayList<Template>();
			List<Element> elements = document.selectNodes("/sram/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(shopxxXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/sram/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File mocXmlFile = new ClassPathResource(CommonAttributes.SRAM_XML_PATH).getFile();
			Document document = new SAXReader().read(mocXmlFile);
			Element element = (Element) document.selectSingleNode("/sram/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
