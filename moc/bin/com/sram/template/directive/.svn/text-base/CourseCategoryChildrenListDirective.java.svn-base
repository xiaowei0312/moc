 ...
 * @version 1.0
 */
@Component("courseCategoryChildrenListDirective")
public class CourseCategoryChildrenListDirective extends BaseDirective {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
		...
		if (courseCategoryId != null && courseCategory == null) {
			...
			if (useCache) {
				courseCategories = courseCategoryService.findChildren(courseCategory, count, cacheRegion);
			} else {
				courseCategories = courseCategoryService.findChildren(courseCategory, count);
			}
		}
