...
public abstract class BaseDaoImpl<T, ID extends Serializable> implements BaseDao<T, ID> {
	...
	public <T> List<T> createQueryPage(String jpql,Integer page,Integer pageSize,List<Object> parameter)
	{
		@SuppressWarnings("unchecked")
		TypedQuery<T> query =(TypedQuery<T>) entityManager.createQuery(jpql,entityClass);
		if(parameter!=null){
	...
	@SuppressWarnings("hiding")
	public  List<Object[]> createNativeQueryPage(String sql,Integer page,Integer pageSize,List<Object> parameter)
	{
		Query nativeQuery = entityManager.createNativeQuery(sql);
		if(parameter!=null){
			for (int i = 0; i < parameter.size(); i++) {
				nativeQuery.setParameter(i+1, parameter.get(i));
			}
		}
	 ...
	 * @return
	 */
	public Long createNativeQueryCount(String sql,List<Object> parameter){
		Query nativeQuery = entityManager.createNativeQuery(sql);
		if(parameter!=null){
			for (int i = 0; i < parameter.size(); i++) {
				nativeQuery.setParameter(i+1, parameter.get(i));
	 ...
	 * @param sql
	 * @param parameter
	 * @return list的Object数组
	 */
	public List<Object[]> createNativeQuery(String sql,List<Object> parameter){
		Query nativeQuery = entityManager.createNativeQuery(sql);
		if(parameter!=null){
			for (int i = 0; i < parameter.size(); i++) {
				nativeQuery.setParameter(i+1, parameter.get(i));
	...
	@SuppressWarnings("unchecked")
	public List<Object[]> createQueryPageObject(String jpql,Integer page,Integer pageSize,List<Object> parameter){
	...
	public Long createQueryCount(String jpql,List<Object> parameter)
	{
		Query query = entityManager.createQuery(jpql);
		if(parameter!=null){
			for (int i = 0; i < parameter.size(); i++) {
	...
	@SuppressWarnings("hiding")
	public <T> List<T> createQuery(String jpql,List<Object> parameter)
	{
		@SuppressWarnings("unchecked")
		TypedQuery<T> query =(TypedQuery<T>) entityManager.createQuery(jpql,entityClass);
		if(parameter!=null){
	...
	public void independentPersist(T entity) {
