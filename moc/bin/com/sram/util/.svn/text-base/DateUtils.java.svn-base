...
public class DateUtils {
	...
	public final static String FORMAT_DATETIME1 = "yyyy-MM-dd HH:mm:ss";
	public final static String FORMAT_DATETIME2 = "yyyyMMddHHmmss";
	...
	public static Date getDate(String dateValue, String dateType)
		...
		if (dateType.equals(TYPE_DATE)) {
			...
			SimpleDateFormat sftime = new SimpleDateFormat(FORMAT_DATETIME1);
	 ...
	 * @dateType 格式化的类型,date和datetime
	 */
	public static String format(Object dateValue, String dateType, String format) {
		if (dateValue == null)
			return "";
		if (dateValue instanceof java.sql.Date) {
			return dateValue.toString();
		} else if (dateValue instanceof java.util.Date) {
			if (dateType.equals(TYPE_DATE) || dateType.equals(TYPE_DATETIME)) {
				java.text.SimpleDateFormat sfdate = new java.text.SimpleDateFormat(
						format);
	...
	public static int getWeekDay(Date date) {
		Date day = DateUtils.dateAdd(format(date, TYPE_DATE, FORMAT_DATE), -1);
		Calendar strDate = Calendar.getInstance();
		strDate.setTime(day);
		int meStrDate = strDate.get(Calendar.DAY_OF_WEEK);
		return meStrDate;
	...
	public static boolean isWeekend(Date date) {
		int weekDay = getWeekDay(format(date, TYPE_DATE, FORMAT_DATE));
		if (weekDay == 6 || weekDay == 7) {
			return true;
		} else {
			return false;
	 ...
	 *            日期列表
	 * @param orderType
	 *            排序类型 <br/>{@link #DATE_ORDER_ASC}<br/> {@link #DATE_ORDER_DESC}
	 * @return 排序结果
	 */
